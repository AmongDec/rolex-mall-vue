        ...
        <div class="content_right">
            ...
            <div   class="content_right-shopMsg">
                ...
                <div class="content_right-shopMsg_left">
                    <img width="205px" height="205px"  :src="'../../../static/img/good/'+goodlist.gid+'/'+goodlist.gmainPic"/>
                </div>
                <!--中:这里的样式是我直接copy过来的，所以class的名称我没有改-->
                <div class="content_left-bottom_msg">
                    <div class="content_left-bottom_msg-up">
                        <!-- 腕表名称 -->
                        <span class="content_left-bottom_msg-up_name" >{{goodlist.gname}}</span> <br/>
                        <span>型号：</span>
                        <span class="content_left-bottom_msg-up_design">{{goodlist.gversion}}</span> <br/>
                        <span>材质：</span>
                        <span class="content_left-bottom_msg-up_design">{{goodlist.gmaterial}}</span>
                    </div>
                ...
                <div class="content_right-shopMsg_right">
                    ...
                    <span class="content_right-shopMsg_right-monny">
								￥
								<span >{{number_format(goodlist.gprice, 2, '.', ',')}}</span>
							</span>
                </div>
                <!--空盒子-->
                <div class="clear"></div>
...
export default {
  ...
  computed:{
    //商品总计
    allprice(){
      return this.number_format(this.goodlist.gprice , 2 , '.' , ',')
    },
    //商品总数量
    allcount(){
      return 1
  ...
  methods:{
    ...
    async createorder(){
      ...
      const sub = {
          ...
          addid: adds,
          gid: this.goodlist.gid
      }
      console.log(sub)
      let oid = ''
      //发送创建订单的网络请求
      try{
        let res = await this.$http.post('order/createone',sub)
...
<style scoped>
  @import "../../assets/css/confirmOrderOne.css";
</style>
